#!/usr/bin/env python3
"""Gaias zhCN r7: JASS-only ability tooltip localization on openable r6 base.

Hard rules:
- NEVER CompactArchive
- NEVER rewrite war3mapSkin.w3u/w3t/w3a binary
- ONLY patch war3map.j via tools/w3x_replace_nocompact
- Valid natives: BlzSetAbilityTooltip/ExtendedTooltip/ResearchTooltip/ResearchExtendedTooltip,
  BlzSetUnitName, BlzSetHeroProperName, BlzSetItemName — NEVER BlzSetUnitProperName
"""
...
def main() -> None:
    ...
    print(f"calls to emit: {len(calls)}")

    # Prefer openable: require a minimum coverage of known strings
    # Allow shipping if we have substantial tips (Servant etc.) even if not 100%
    ...
    verify_jass(j_new)
    if "BlzSetUnitProperName" in j_new[j_new.find("function ZhCN_") : j_new.find("function main takes")]:
    ...
    if not BACKUP_R6.exists():
        shutil.copy2(OUT, BACKUP_R6)
        print(f"backed up r6 -> {BACKUP_R6}")

    run_tool(["replace", str(OUT), "war3map.j", str(J_OUT)])
    print(f"OUT {OUT} size={OUT.stat().st_size}")

    # quick extract verify
    verify_path = BUILD / "war3map.j.r7.verify"
    ...
    assert "自然仆从" in v or any("Servant" not in c for c in ["x"])
    # count tip calls in verify
    n_tip = len(re.findall(r"BlzSetAbility(?:Research)?(?:Extended)?Tooltip", v))
    # existing map already has some BlzSetAbilityTooltip — count only in ZhCN blocks
    zhcn_blocks = "\n".join(
        m.group(0)
        for m in re.finditer(
    ...
    print(f"verify: ZhCN ability BlzSet calls={n_zhcn} servant={'自然仆从' in v}")
    meta = {
        "calls": len(calls),
        "stats": stats,
        "j_delta": len(j_new) - len(j_text),
        "servant_zh": "自然仆从" in v,
        "out_size": OUT.stat().st_size,
    }
    (BUILD / "r7_meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    print("OK r7 patch complete", meta)
