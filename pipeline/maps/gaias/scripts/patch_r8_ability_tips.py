#!/usr/bin/env python3
"""Gaias zhCN r8: fix ability hover/learn tips (levels + research + reapply).

Extends r7 tip generation:
- Duplicate every tip to level 0 AND 1 (Reforged indexing quirk)
- Learn/atp1 learnables also set ResearchTooltip + ResearchExtendedTooltip
  (AZ* learn cards often lack aret/arut in Skin.w3a)
- Re-apply ZhCN_AbilityTips at 0.5 / 2.0 / 5.0 once AND every 10s;
  ZhCN_RenameAll also calls ZhCN_AbilityTips (class select overwrite)
...
def main() -> None:
    ...
    # Preserve r7 artifact if not already saved
    ...
    verify_path = BUILD / "war3map.j.r8.verify"
    ...
    assert "自然仆从" in v
    assert "call TimerStart(CreateTimer(),2.,false,function ZhCN_AbilityTips)" in v
    assert "call TimerStart(CreateTimer(),5.,false,function ZhCN_AbilityTips)" in v
    assert "call TimerStart(CreateTimer(),10.,true,function ZhCN_AbilityTips)" in v
    ...
    (BUILD / "r8_meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    # also copy r8 map snapshot
    snap = ROOT / "gaias/out/GaiasORPG_v1_3A_12_zhCN-r8.w3x"
    shutil.copy2(OUT, snap)
    print("OK r8 patch complete", meta)
